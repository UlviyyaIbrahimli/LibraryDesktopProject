 ...
 * @Ulviyye Ibrahimli
 */
public class UpdateBookShelfFrame extends javax.swing.JFrame {
    ...
    public UpdateBookShelfFrame() {
        initComponents();
    }

    UpdateBookShelfFrame(DepartmentService departmentService,
        ...
        try {
            ...
            this.languageService = languageService;
            initComponents();
            roomFloorCombo.setRenderer(new UpdateBookShelfFrame.TwoDecimalRenderer(roomFloorCombo.getRenderer()));
            roomNumberCombo.setRenderer(new UpdateBookShelfFrame.TwoDecimalRenderer(roomNumberCombo.getRenderer()));
            hideWarning();
            ...
            BookShelf bookShelf = bookShelfService.getBookShelfById(selectedId);
            showOldDate(bookShelf);
            String floor = String.valueOf(bookShelf.getRoom().getFloor());
            roomFloorCombo.setSelectedItem(floor);
            showRoomNumberComboSelected(bookShelf, floor);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void initComponents() {
        ...
        shelfFloorTxt.setEditable(false);
        ...
        shelfFloorTxt.setEnabled(false);
        ...
        potensialCapacityTxt.setEditable(false);
        ...
        potensialCapacityTxt.setEnabled(false);
        ...
        jLabel8.setText("Update Shelf of Book");
        ...
        shelfWarningLbl.setFont(new java.awt.Font("Times New Roman", 3, 12)); // NOI18N
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                            .addComponent(roomNumberCmbLbl, javax.swing.GroupLayout.DEFAULT_SIZE, 119, Short.MAX_VALUE)
                        ...
                        .addGap(260, 260, 260)
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(roomNumberCombo, javax.swing.GroupLayout.PREFERRED_SIZE, 241, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(roomFloorCombo, javax.swing.GroupLayout.Alignment.TRAILING, javax.swing.GroupLayout.PREFERRED_SIZE, 241, javax.swing.GroupLayout.PREFERRED_SIZE)))
                    .addGroup(javax.swing.GroupLayout.Alignment.LEADING, jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                            ...
                            .addComponent(shelfWarningLbl, javax.swing.GroupLayout.DEFAULT_SIZE, 119, Short.MAX_VALUE)
                            .addComponent(capacityWarningLbl, javax.swing.GroupLayout.Alignment.LEADING, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                        .addGap(50, 50, 50)
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanel1Layout.createSequentialGroup()
                            ...
                            .addComponent(shelfNumberTxt)
                            .addComponent(shelfFloorTxt)
                            .addComponent(potensialCapacityTxt))))
                .addGap(31, 31, 31))
        );
    ...
    private void shelfNumberTxtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_shelfNumberTxtActionPerformed
        shelfNumberTxt.setForeground(Color.MAGENTA.darker().darker().darker());
    ...
    private void shelfFloorTxtMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_shelfFloorTxtMouseClicked
        shelfFloorTxt.setForeground(Color.MAGENTA.darker().darker().darker());
        shelfFloorWarninLbl.setVisible(false);
    ...
    private void roomFloorComboItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_roomFloorComboItemStateChanged
        ...
        if (evt.getStateChange() == 1) {
            if (roomFloorCombo.getSelectedIndex() > 0) {
                showRoomNumberCombo(selectedFloor);
            } else {
        ...
        try {
            ...
            if (shelfNumber.matches("[a-zA-Z0-9\\s]+") && (!shelfNumber.isEmpty())) {
                ...
                if (roomFloorCombo.getSelectedIndex() > 0) {
                    ...
                    if ((!roomNumberCombo.getSelectedItem().equals("There is no book-room on this floor")) && (!roomNumberCombo.getSelectedItem().equals("Select Room Number"))) {
                        Item roomNumber = (Item) roomNumberCombo.getSelectedItem();
                        String potCap = potensialCapacityTxt.getText();
                        boolean check = checkShelfNumberUnique(shelfNumber, roomNumber.getId());
                        if (check) {
                            BookShelf bookShelf = new BookShelf();
                            bookShelf.setShelfFloor(Integer.parseInt(shelfFloor));
                            bookShelf.setShelfNumber(shelfNumber);
                            bookShelf.setPotensialCapacity(Integer.parseInt(potCap));
                            BookRoom bookRoom = new BookRoom();
                            bookRoom.setFloor(Integer.parseInt(roomFloor));
                            bookRoom.setId(roomNumber.getId());
                            bookShelf.setRoom(bookRoom);
                            boolean isUpdateBookShelf = bookShelfService.updateBookShelf(bookShelf, selectedId);
                            if (isUpdateBookShelf) {
                                JOptionPane.showMessageDialog(null, "UPDATE IS SUCCESSFUL!", "SUCCES", JOptionPane.INFORMATION_MESSAGE, new ImageIcon("C:\\Users\\user\\Desktop\\java/tick.png"));
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        int choose = JOptionPane.showConfirmDialog(null, "Are you sure close prrogram?");
    ...
    private void showOldDate(BookShelf bookShelf) {
        shelfFloorTxt.setText(String.valueOf(bookShelf.getShelfFloor()));
        shelfNumberTxt.setText(bookShelf.getShelfNumber());
        potensialCapacityTxt.setText(String.valueOf(bookShelf.getPotensialCapacity()));
    }

    private void showRoomNumberComboSelected(BookShelf bookShelf, String floor) {
        try {
            DefaultComboBoxModel model = (DefaultComboBoxModel) roomNumberCombo.getModel();
            List<BookRoom> bookRoomList = bookRoomService.getRoomNumburForFloor(Integer.parseInt(floor));
            model.removeAllElements();
            for (BookRoom bookRoom : bookRoomList) {
                model.addElement(new Item(bookRoom.getId(), bookRoom.getNumber()));
            }
            model.setSelectedItem(new Item(bookShelf.getRoom().getId(), bookShelf.getRoom().getNumber()));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }

    private void showRoomNumberCombo(String selectedFloor) {
        try {
            DefaultComboBoxModel model = (DefaultComboBoxModel) roomNumberCombo.getModel();
            List<BookRoom> bookRoomList = bookRoomService.getRoomNumburForFloor(Integer.parseInt(selectedFloor));
            model.removeAllElements();
    ...
    boolean checkShelfNumberUnique(String number, Long roomId) {
        boolean result = true;
        try {
            BookShelf shelf = bookShelfService.getBookShelfById(selectedId);
            if (!shelf.getShelfNumber().equals(number)) {
                List<BookShelf> bookShelfs = bookShelfService.getBookShelfByRoomId(roomId);
                for (BookShelf bookShelf : bookShelfs) {
                    if (bookShelf.getShelfNumber().equals(number)) {
                        result = false;
                    }
                }
            }
