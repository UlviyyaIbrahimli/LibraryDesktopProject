...
public class BookDao implements IBook {
    ...
    public List<Book> allBookList(int choose) throws Exception {
        ...
        String sql2
                = "    SELECT ROW_NUMBER () OVER (ORDER BY b.id) AS r,\n"
                + "         B.ID,\n"
                + "         B.ISBN,\n"
                + "         B.TITLE,\n"
                + "         B.B_EDITION,\n"
    ...
    public Float getCount(int choose, Long id) throws Exception {
        ...
        String sql2 = "SELECT COUNT (BOOK_ID) as book_count_By_Id\n"
                + "    FROM BOOK_SUBJECT  where active=1\n"
                + "GROUP BY SUBJECT_ID\n"
                + "  HAVING SUBJECT_ID = ? ";
        ...
        String sql32 = "select count(B.ALL_COUNT) as allBooks from book b where b.active=1";
    ...
    public List<Book> bookBySubjectId(Long subjectId) throws Exception {
        ...
        String sql = "  SELECT ROWNUM R,\n"
                ...
                + "                    ON S.ID = BS.SUBJECT_ID\n"
                + "           WHERE BS.SUBJECT_ID = ? AND b.active = 1\n"
    ...
    public List<Book> getSubjectByBookId(Long bookId) throws Exception {
        ...
        String sql = "SELECT  distinct S.ID, S.SUBJECT_NAME\n"
                + "  FROM BOOK_SUBJECT BSS\n"
                + "       INNER JOIN BOOK B\n"
                + "          ON B.ID = BSS.BOOK_ID\n"
                + "       INNER JOIN SUBJECTS S\n"
                + "          ON S.ID = BSS.SUBJECT_ID\n"
                + " WHERE bss.ACTIVE = 1 AND B.ID = ?";
